...
public class UtPLSQLMojoTest {
     ...
     * 
     * Given : a pom.xml with invalid sources directory When : pom is read and buildSourcesOptions is run Then : it
     * should throw a MojoExecutionException
     */
    @Test
     ...
     * 
     * Given : a pom.xml with invalid tests directory When : pom is read and buildTestsOptions is run Then : it should
     ...
     * 
     * Given : a pom.xml with sources and tests with a lot of parameters When : pom is read and buildSourcesOptions /
     * buildTestsOptions are run Then : it should fill all parameters correctly
     */
    @Test
     ...
     * 
     * Given : a pom.xml with no sources / tests tags and default directory does not exist. When : pom is read and
     * buildSourcesOptions / buildTestsOptions are run Then : it should not find any source files
     */
    @Test
     ...
     * 
     * Given : a pom.xml with no sources / tests tags but default directory exists. When : pom is read and
     * buildSourcesOptions / buildTestsOptions are run Then : it should find all sources/tests files in default
     * directories
     */
    @Test
     ...
     * 
     * Given : a pom.xml with source and test tag not containing a directory tag. When : pom is read and
     * buildSourcesOptions / buildTestsOptions are run Then : it should find all sources/tests files in default
     * directories
     */
    @Test
    ...
    @Test
    public void testNoDeclaredReportersShouldUseDefaultReporter() throws Exception {
        // Prepare mocks
        ReporterFactory reporterFactory = Mockito.mock(ReporterFactory.class);
        Connection mockConnection = Mockito.mock(Connection.class);
        Reporter reporter = Mockito.mock(Reporter.class);
        Mockito.when(reporter.init(Mockito.any())).thenReturn(reporter);
        Mockito.when(reporterFactory.createReporter("UT_DOCUMENTATION_REPORTER")).thenReturn(reporter);

        // load pom.xml
        UtPLSQLMojo utplsqlMojo = (UtPLSQLMojo) rule
                .lookupConfiguredMojo(new File("src/test/resources/testReportersParam/noparams/"), "test");
        Assert.assertNotNull(utplsqlMojo);
        
        // check state before running initReporters.
        List<ReporterParameter> reporterParameters = Whitebox.getInternalState(utplsqlMojo, "reporters");
        assertEquals(0, reporterParameters.size());

        // execute initReporters
        List<Reporter> reporters = Whitebox.invokeMethod(utplsqlMojo, "initReporters", mockConnection,
                new Version("1.2.3"), reporterFactory);

        // reporters property should have been updated with default reporter
        reporterParameters = Whitebox.getInternalState(utplsqlMojo, "reporters");
        assertEquals(1, reporterParameters.size());
        assertEquals("UT_DOCUMENTATION_REPORTER", reporterParameters.get(0).getName());
        assertFalse(reporterParameters.get(0).isFileOutput());
        assertTrue(reporterParameters.get(0).isConsoleOutput());

        // initReporters returns one reporter
        assertEquals(1, reporters.size());
        
        // reporterWriter has one reporterWriter
        ReporterWriter reporterWriter = Whitebox.getInternalState(utplsqlMojo, "reporterWriter");
        Whitebox.getInternalState(utplsqlMojo, "reporterWriter");
